//
//  CFPopUpDonProductViewController.m
//  CheckFood
//
...
@interface CFPopUpDonProductViewController ()

@end

@implementation CFPopUpDonProductViewController
...
- (IBAction)validerButtonAction:(id)sender
{
    ...
    if ([self.emplacement isEqualToString:@"monPlacard"]) {
        ...
        if ([prefVC isKindOfClass:[CFMyCupBoardViewController class]]) {
            [prefVC donProductAction:self.donButton];
        }
    }
            ...
            if ([prefVC isKindOfClass:[CFAlertesViewController class]]) {
                [prefVC GiveProductAction:self.donButton];
            }
        }
