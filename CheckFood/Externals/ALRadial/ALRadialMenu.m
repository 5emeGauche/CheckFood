...
- (void)itemsWillAppearFromButton:(UIButton *) button withFrame:(CGRect)frame inView:(UIView *)view isMenu:(BOOL)isMenu{
	if ([self.items count]) {
 ...
 //   float angle = (arc/(itemCount-1));
    ...
    [view insertSubview:self.overlayView belowSubview:button];

    if (isMenu) {
        
        self.scanLabel = [[UILabel alloc] initWithFrame:CGRectMake(77, 295, 150, 20)];
    }
    else {
    
    if (IS_IPHONE_5) {
        self.scanLabel = [[UILabel alloc] initWithFrame:CGRectMake(77, 363, 150, 20)];
    }
    else {
        
        self.scanLabel = [[UILabel alloc] initWithFrame:CGRectMake(77, 275, 150, 20)];
    }
        
    }
    self.scanLabel.text = @"Scanner";
    [self.scanLabel setFont:[UIFont fontWithName:@"Roboto-Regular" size:13]];
    [self.scanLabel setTextColor:[UIColor whiteColor]];
    
    if (isMenu) {
        self.histLabel = [[UILabel alloc] initWithFrame:CGRectMake(187, 295, 150, 20)];
    }
    else {
    if (IS_IPHONE_5) {
        self.histLabel = [[UILabel alloc] initWithFrame:CGRectMake(187, 363, 150, 20)];
    }
    else {
        self.histLabel = [[UILabel alloc] initWithFrame:CGRectMake(187, 275, 150, 20)];
    }
        
    }
    
    self.histLabel.text = @"Historique";
    [self.histLabel setFont:[UIFont fontWithName:@"Roboto-Regular" size:13]];
    [self.histLabel setBackgroundColor:[UIColor clearColor]];
    [self.scanLabel setBackgroundColor:[UIColor clearColor]];
    [self.histLabel setTextColor:[UIColor whiteColor]];

    
    [view insertSubview:self.scanLabel belowSubview:button];
    [view insertSubview:self.histLabel belowSubview:button];
    // [view addSubview:self.scanLabel];
     //[view addSubview:self.histLabel];
    
    self.scanLabel.hidden = YES;
    self.histLabel.hidden = YES;
    
    NSTimer* timer = [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(hideLabel) userInfo:nil repeats:NO];
    
	int currentItem = 1;
	ALRadialButton *popupButton;
	while (currentItem <= itemCount) {
     //   float radians = (angle * (currentItem - 1) + start) * (M_PI/180);
     //   float radians = (angle * (currentItem - 1) + 20) * (M_PI);
		
	//	int x = round (centerX + radius * cos(radians));
	//	int y = round (centerY + radius * sin(radians));
    //    int x = round (centerX + radius * cos(radians) - 100);
	//	int y = round (centerY+radius * sin(radians) - 200);
      ...
      //  int extraX = round (centerX + (radius*2) * cos(radians) -100);
	//	int extraY = round (centerY + (radius*2) * sin(radians) -200);
        ...
        CGRect frame = CGRectMake(centerX-buttonSize*0.5f, centerY-buttonSize*0.5f, 70, 70);
		
        CGPoint final;
        CGPoint bounce;
        if (currentItem == 1) {
            /*  final = CGPointMake(185, 350);
            bounce = CGPointMake(185, 350);*/
            if (isMenu) {
                
                final = CGPointMake(217, 350);
                bounce = CGPointMake(217, 350);
            }
            else {
            
            if (IS_IPHONE_5) {
                
                final = CGPointMake(217, 418);
                bounce = CGPointMake(217, 418);
            }
            else {
                
                final = CGPointMake(217, 330);
                bounce = CGPointMake(217, 330);
            }
            }
         
        }
        else  if (currentItem == 2) {
          /*  final = CGPointMake(70, 350);
            bounce = CGPointMake(70, 350);*/
            
            if (isMenu) {
                
                final = CGPointMake(102, 350);
                bounce = CGPointMake(102, 350);
            }
            else {
            
            if (IS_IPHONE_5) {
                final = CGPointMake(102, 418);
                bounce = CGPointMake(102, 418);
            }
            else {
                
                final = CGPointMake(102, 330);
                bounce = CGPointMake(102, 330);
            }
            }
     ...
     //   CGPoint final = CGPointMake(extraX, extraY);
...
- (void)buttonsWillAnimateFromButton:(id)sender withFrame:(CGRect)frame inView:(UIView *)view isMenu:(BOOL)isMenu{
	...
	if ([self.items count]) {
		...
		[self itemsWillAppearFromButton:sender withFrame:frame inView:view isMenu:isMenu];
	}
}
...
- (void) buttonDidFinishAnimation:(ALRadialButton *)radialButton {
    ...
    [self.histLabel removeFromSuperview];
